...
export default function ViewReservationDetails() {
  ...
  const [editedReservationCategory, setEditedReservationCategory] = useState();
  ...
  const { fullName, email, address, height, weight, bmi, gender } = patientData;
  ...
  async function handleUpdate(e) {
    ...
    const tempObj = {
      ...
      reservationDate: formatDateAndTime(editedReservationDate).date,
    };
    console.log("Edited Datea", tempObj);

    // api must be here
    const response = await HospitalApi.put("/update-reservation", tempObj, {
      headers: {
            ...
            <td>
              {enableEdit ? (
                <select
                  ...
                  onChange={(e) => {
                    setEditedReservationType(e.target.value);
                  }}
                >
            ...
            <td>
              {enableEdit && editedReservationType ? (
                <select
                  ...
                  value={editedReservationType}
                  onChange={(e) => {
                    setEditedReservationCategory(e.target.value);
                  }}
                >
                  {hospital?.[editedReservationType]?.map((category, index) => (
            ...
            <td>
              {enableEdit ? (
                <input
                  ...
                  onChange={(e) => {
                    setEditedReservationDate(e.target.value);
                  }}
                />
              ) : (
                formatDateAndTime(reservation.reservationDate).date
              )}
            </td>
          </tr>
        </table>
        {enableEdit ? (
          <input className={`btn`} type="submit" value="Confirm Edit" />
        ) : (
          <button
            className={`btn`}
